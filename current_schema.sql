...
CREATE POLICY "Admins can invite members to their organization" ON "public"."profiles" FOR INSERT WITH CHECK ((("user_id" IS NULL) AND ("organization_id" IN ( SELECT "profiles_1"."organization_id"
   FROM "public"."profiles" "profiles_1"
  WHERE (("profiles_1"."user_id" = "auth"."uid"()) AND (("profiles_1"."role")::"text" = 'admin'::"text"))))));
...
CREATE POLICY "Admins can view all profiles" ON "public"."profiles" FOR SELECT USING ((("user_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "admin_check"
  WHERE (("admin_check"."user_id" = "auth"."uid"()) AND (("admin_check"."role")::"text" = 'admin'::"text"))))));
...
CREATE POLICY "Allow profile creation for invited users" ON "public"."profiles" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."member_invitations"
  WHERE ((("member_invitations"."email")::"text" = ("profiles"."email")::"text") AND (("member_invitations"."status")::"text" = 'pending'::"text") AND ("member_invitations"."expires_at" > "now"())))));
...
CREATE POLICY "Trainers can view org profiles" ON "public"."profiles" FOR SELECT USING ((("user_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "trainer_check"
  WHERE (("trainer_check"."user_id" = "auth"."uid"()) AND (("trainer_check"."role")::"text" = 'trainer'::"text") AND ("trainer_check"."organization_id" = "profiles"."organization_id"))))));
